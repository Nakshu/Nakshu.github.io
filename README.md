...
1. **Add your résumé.** Export your résumé as a PDF, name it exactly
   `resume.pdf`, and place it in this same folder (next to `index.html`).
   The "Download résumé" buttons link to that filename.
2. **Double-check the links** in `index.html` point to your real GitHub
   repos, LinkedIn, and email — they're already filled in, but confirm the
   URLs are correct: search for `github.com/Nakshu`, `linkedin.com/in/`, and
   `mailto:`.
